...
const StatItem = ({ value, label, colorClass }) => (
  <div className="text-center">
    <div className={`text-4xl font-extrabold ${colorClass} mb-1 transition-transform transform hover:scale-105`}>
      {value}
    </div>
...
const FooterLink = ({ href, children }) => {
  ...
  const handleClick = (e) => {
    if (href.startsWith('#')) {
      // If it's an anchor link, prevent default navigation
      e.preventDefault();
      ...
      // For all other links, use react-router navigation
      navigate(href);
    }
  };
  ...
  return (
    <button 
      onClick={handleClick}
      className="text-sm text-muted-foreground hover:text-primary transition-colors block mb-2 text-left"
    >
      {children}
    </button>
  );
...
const Landing = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="py-12 bg-card">
        <div className="container mx-auto px-6">
          <div className="flex justify-around items-center flex-wrap gap-8">
            <StatItem value="10K+" label="Resumes Created" colorClass="text-[#1A5C94]" />
            <StatItem value="95%" label="Approval Rate" colorClass="text-[#1A5C94]" />
            <StatItem value="24h" label="Average Response" colorClass="text-[#32CD32]" />
            <StatItem value="8K+" label="Happy Users" colorClass="text-[#1A5C94]" />
          </div>
        </div>
      </section>

      {/* About Section (Target for About Us Scroll) */}
      ...
      {/* Testimonials Section (Target for Reviews Scroll) */}
      ...
      <footer className="bg-card pt-16 pb-8 border-t">
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
            ...
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xl font-bold text-primary">Resume Builder Pro</span>
              </div>
              <p className="text-sm text-muted-foreground">
                Building careers one resume at a time. Professional, fast, and approved.
              </p>
            </div>
          ...
          {/* Horizontal Line and Developer Credit */}
          <div className="border-t pt-4 mt-4 text-center text-xs text-muted-foreground/70">
