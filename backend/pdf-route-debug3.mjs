...
(async () => {
  ...
  const info = await page.evaluate(() => {
    ...
    return {
      ...
      themeRootCount: document.querySelectorAll('.resume-theme-root').length,
      shellOuterHTMLLength: shell?.outerHTML.length,
      resumeScaleExists: Boolean(document.querySelector('.resume-document-scale')),
      shellChildrenCount: shell?.children.length,
      shellChildren: shell ? Array.from(shell.children).map((c) => ({ tag: c.tagName, className: c.className })) : null,
      pageOuterHTML: document.querySelector('.resume-page')?.outerHTML.slice(0, 1000) || null,
      headers: Array.from(document.querySelectorAll('.resume-document-shell header')).map((h) => ({ text: h.textContent?.trim().slice(0,100), className: h.className }))
    };
  });
  console.log(JSON.stringify(info, null, 2));
  await page.screenshot({ path: 'pdf-route-debug3.png', fullPage: true });
  await browser.close();
})();
